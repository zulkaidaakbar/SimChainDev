...
for (int ii=1; ii<=1000; ii++)
{
 sprintf(name_file,"/pnfs/e906/persistent/users/zakbar/SimChainDev/july9_DAFREF/%i/out/trk_eval.root",ii);
ttree->Add(name_file);
cerr<<name_file<<endl;
}
...
ttree->Add("/e906/app/users/zakbar/July2020/e1039-analysis/SimChainDev/scratch/july13test/1/out/trk_eval.root");
...
TH1D *h_dreco_x_st1 = new TH1D("h_dreco_x_st1", "h_dreco_x_st1", 50,-10,10);
h_dreco_x_st1->Sumw2();

TH1D *h_dtracklet_x_st1 = new TH1D("h_dtracklet_x_st1", "h_dtracklet_x_st1", 50,-0.5,0.5);
h_dtracklet_x_st1->Sumw2();

TH1D *h_dreco_y_st1 = new TH1D("h_dreco_y_st1", "h_dreco_y_st1", 50,-2,2);
h_dreco_y_st1->Sumw2();

TH1D *h_dtracklet_y_st1 = new TH1D("h_dtracklet_y_st1", "h_dtracklet_y_st1", 50,-0.5,0.5);
   ...
   for (int i=0; i<n_tracks; i++)
    {
     ...
     h_dreco_p_st1->Fill((1.0/truth_p_st1 - 1.0/reco_p_st1)/TMath::Sqrt(covSt1_0[i]));
     h_dreco_x_st1->Fill((truth_x_st1 - reco_x_st1)/TMath::Sqrt(covSt1_3[i]));
     h_dreco_y_st1->Fill((truth_y_st1 - reco_y_st1)/TMath::Sqrt(covSt1_4[i]));
	...
	cerr<<TMath::Sqrt(covSt1_3[i])<<endl;
     if (Test_file)
      {
       cerr<<covSt1_4[i]<<"   "<<uncSt1_4[i]<<endl;
      }
...
c1->cd();  
	h_dreco_p_st1->SetTitle("#Delta P at st1 ;#Delta P; N");
			...
			c1->SaveAs("pull_hist_dreco_p_st1.png");
...
c2->cd();  
	h_dreco_x_st1->SetTitle("#Delta X at st1 ;#Delta X; N");
			...
			c2->SaveAs("pull_hist_dreco_x_st1.png");
...
c3->cd();  
	h_dreco_y_st1->SetTitle("#Delta Y at st1 ;#Delta Y; N");
			...
			c3->SaveAs("pull_hist_dreco_y_st1.png");
