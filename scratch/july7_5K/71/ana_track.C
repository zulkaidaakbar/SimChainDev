...
TH1D *h_dpx = new TH1D("h_dpx","h_dpx",100,-0.5,0.5);
h_dpx->Sumw2();

TH1D *h_dpy = new TH1D("h_dpy","h_dpy",100,-0.5,0.5);
h_dpy->Sumw2();

TH1D *h_dpz = new TH1D("h_dpz","h_dpz",100,-5,5);
h_dpz->Sumw2();
   ...
   for (int i=0; i<n_tracks; i++)
    {
     ...
     //double mom_truth = TMath::Sqrt(gpx_st1[i]*gpx_st1[i]+gpz_st1[i]*gpz_st1[i]);
...
//
    double track_p = 1.0/fabs(stateSt1_0[i]);
    double track_pz = track_p/sqrt(1. + stateSt1_1[i]*stateSt1_1[i] + stateSt1_2[i]*stateSt1_2[i]);
    double track_px = track_pz*stateSt1_1[i];
    double track_py = track_pz*stateSt1_2[i];
//
...
p_dist->Fill(mom_truth,(gpz_st1[i] - track_pz));
        hprof->Fill(mom_truth,(gpz_st1[i] - track_pz));
	h_dpx->Fill(gpx_st1[i] - track_px);
	h_dpy->Fill(gpy_st1[i] - track_py);
	h_dpz->Fill(gpz_st1[i] - track_pz);
       }
     if (delta_tx != 0)
      {  
...
c1->cd();  
	pull_mom->SetTitle("#Delta P; #Delta P; N");
...
c11->cd();
	...
	p_dist->SetTitle("; Pz; #Delta Pz");
                        ...
                        p_dist->Draw("COLZ");
                        //recstat->SetStats(0);
                        c11->SaveAs("p_dist_z.png");

TCanvas *c12 = new TCanvas("c12","c12"); c12->SetGrid();
c12->cd();
hprof->Draw();
c12->SaveAs("hprof_z.png");

TCanvas *c13 = new TCanvas("c13","c13"); c13->SetGrid();
c13->cd();  
	h_dpx->SetTitle("#Delta Px; #Delta Px; N");
			h_dpx->SetMarkerColor(kRed);
                        h_dpx->SetMarkerStyle(20);
                        h_dpx->SetMarkerSize(1);
			h_dpx->SetLineColor(kRed);
			h_dpx->Draw("e");
			c13->SaveAs("h_dpx.png");

TCanvas *c14 = new TCanvas("c14","c14"); c14->SetGrid();
c14->cd();  
	h_dpy->SetTitle("#Delta Py; #Delta Py; N");
			h_dpy->SetMarkerColor(kRed);
                        h_dpy->SetMarkerStyle(20);
                        h_dpy->SetMarkerSize(1);
			h_dpy->SetLineColor(kRed);
			h_dpy->Draw("e");
			c14->SaveAs("h_dpy.png");

TCanvas *c15 = new TCanvas("c15","c15"); c15->SetGrid();
c15->cd();  
	h_dpz->SetTitle("#Delta Pz; #Delta Pz; N");
			h_dpz->SetMarkerColor(kRed);
                        h_dpz->SetMarkerStyle(20);
                        h_dpz->SetMarkerSize(1);
			h_dpz->SetLineColor(kRed);
			h_dpz->Draw("e");
			c15->SaveAs("h_dpz.png");
